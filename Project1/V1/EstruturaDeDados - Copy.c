...
// Gravar toda a informa��o para ficheiro
int Grava()
{
	FILE *f;
	f = fopen("save.txt", "w");
	...
	{
		fprintf(f, "----- ALUNOS -----\n");
		fprintf(f, "Numero:\t Pais:\t Nome:\n");
		while (AlListaInternal != NULL)
		{
			fprintf(f, "%d\t %s\t %s\n", AlListaInternal->AlNum, AlListaInternal->AlPais, AlListaInternal->AlNome);
			AlListaInternal = AlListaInternal->AlNext;
		}
		fprintf(f, "----- UC -----\n");
		fprintf(f, "Numero:\t Ano:\t Sem.:\t ECT:\t Est.:\t Nome:\n");
		while (UcListaInternal != NULL)
		{
			fprintf(f, "%d\t %d\t %d\t %d\t %d\t %s\n", UcListaInternal->UcNum, UcListaInternal->UcAno, UcListaInternal->UcSemestre, UcListaInternal->UcECTS, UcListaInternal->UcEstado, UcListaInternal->UcNome);
			UcListaInternal = UcListaInternal->UcNext;
		}
		fprintf(f, "----- INSCRICOES -----\n");
		printf("Numero:\t UC:\t Ano:\t Estado:\n");
		while (InListaInternal != NULL)
		{
			fprintf(f, "%d\t %d\t %d\t %d\n", InListaInternal->InAluno, InListaInternal->InUC, InListaInternal->InUC, InListaInternal->InEstado);
...
const char* GravaCSV()
{
	...
	if (f != NULL)
	{
		...
		strcat(msg,"Erro ao gravar para o ficheiro saveAlunos.csv.\n");
	...
	if (f != NULL)
	{
		while (UcListaInternal != NULL)
		{
			if(UcListaInternal->UcEstado!=0)
				fprintf(f, "%d;%d;%d;%d\n", UcListaInternal->UcNum, UcListaInternal->UcAno, UcListaInternal->UcSemestre, UcListaInternal->UcECTS);
			UcListaInternal = UcListaInternal->UcNext;
		}
		fclose(f);
	}
	else
		strcat(msg, "Erro ao gravar para o ficheiro saveUCs.csv.\n");
	...
	if (f != NULL)
	{
		...
		strcat(msg, "Erro ao gravar para o ficheiro saveInscricoes.csv.\n");

	strcat(msg, "Concluido.");
	return msg;
}
